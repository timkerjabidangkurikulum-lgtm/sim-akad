// Database struktur tema dan opsi pembelajaran - EXPANDED VERSION (tema dasar)
const baseLearningDatabase = {
...
// ===== Penyederhanaan menjadi 9 tema besar =====

// Helper untuk menggabungkan dan menghapus duplikasi array
...
// Pemetaan tema baru -> daftar tema lama yang digabung
const groupedThemeMap = {
  "Jati Diri dan Kebangsaan": [
    "Aku Cinta Alam Ciptaan Allah",
    "Bhineka Tunggal Ika",
    "Menjaga Tradisi, Merangkai Inovasi",
    "Berbeda Itu Indah",
  ...
  "Kepemimpinan & Keterlibatan Sosial": [
    "Gotong Royong Membangun Sekolahku",
    "Suara Demokrasi",
    "Gerakan Filantropi Sosial",
    "Berani Jujur Hebat"
  ...
  "Lingkungan, Energi & Keseimbangan Alam": [
    "Hidup Bersih, Hati Jernih",
    "Gaya Hidup Berkelanjutan",
    "Robot Cerdas & Sensor Lingkungan",
    "Bumi Memanas, Kita Beraksil",
  ...
  "Literasi, Bahasa, dan Kebudayaan": [
    "Seni dan Budaya Digital",
    "Literasi dan Penguatan Bahasa"
  ],
  "Sains, Teknologi, dan Rekayasa Dasar": [
    "Sistem Tubuh Manusia",
    "Robot Sederhana & Mekanisme Gerak",
    "Teknologi Cerdas, Generasi Beretika",
    "Sains Terpadu: Menyelami Rahasia Alam",
...
// Bentuk learningDatabase baru berbasis penggabungan tema-tema dasar
const learningDatabase = {};

Object.entries(groupedThemeMap).forEach(([newName, baseThemes]) => {
  const collected = {
    pokok_bahasan: [],
    mapel_kolaboratif: [],
    topik_kbc: [],
  ...
  baseThemes.forEach((themeKey) => {
    const src = baseLearningDatabase[themeKey];
    if (!src) return;
    collected.pokok_bahasan.push(src.pokok_bahasan);
    collected.mapel_kolaboratif.push(src.mapel_kolaboratif);
    collected.topik_kbc.push(src.topik_kbc);
    collected.profil_lulusan.push(src.profil_lulusan);
    collected.tujuan_pembelajaran.push(src.tujuan_pembelajaran);
    collected.aktivitas_pembelajaran.push(src.aktivitas_pembelajaran);
    collected.integrasi_kbc.push(src.integrasi_kbc);
    collected.penilaian.push(src.penilaian);
  });

  learningDatabase[newName] = {
    pokok_bahasan: mergeUnique(collected.pokok_bahasan),
    mapel_kolaboratif: mergeUnique(collected.mapel_kolaboratif),
    topik_kbc: mergeUnique(collected.topik_kbc),
    profil_lulusan: mergeUnique(collected.profil_lulusan),
    tujuan_pembelajaran: mergeUnique(collected.tujuan_pembelajaran),
    aktivitas_pembelajaran: mergeUnique(collected.aktivitas_pembelajaran),
    integrasi_kbc: mergeUnique(collected.integrasi_kbc),
    penilaian: mergeUnique(collected.penilaian)
  };
});

// Export untuk digunakan (Node/CommonJS, bila diperlukan)
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { learningDatabase, baseLearningDatabase };
}
